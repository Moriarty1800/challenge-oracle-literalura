package com.example.literalura.repository;

import com.example.literalura.modelo.autores;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.data.jpa.repository.Query;
import java.util.List;
import java.util.Optional;

public interface AutorRepository extends JpaRepository<autores, Long> {

    @Query("SELECT a FROM Autor a WHERE a.fechaDeNacimiento <= :fecha AND (a.fechaDeFallecimiento IS NULL OR a.fechaDeFallecimiento > :fecha)")
    List<autores> buscarAutoresVivosEnDeterminadaFecha(Integer fecha);

    Optional<autores> findByNombreIgnoreCase(String nombre);
    List<autores> findByFechaDeNacimiento(Integer fecha);
    Optional<autores> findByNombreContainsIgnoreCase(String nombreAutor);
}